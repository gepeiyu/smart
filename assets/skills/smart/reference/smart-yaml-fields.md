...
Project-level defaults live in `.smart/config.yaml`.

| Field | Meaning |
|-------|---------|
| `smart_language` | `en` or `zh`. Controls the default language for OpenSpec artifacts and `docs/superpowers/` documents generated by Smart workflows |
| `context_compression` | `off` or `beta` |
| `review_mode` | `off`, `standard`, or `thorough` |
