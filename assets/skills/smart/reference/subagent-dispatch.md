...
```bash
"$SMART_BASH" "$SMART_STATE" task-checkoff "$PLAN_FILE" "$PLAN_TASK_TEXT"
"$SMART_BASH" "$SMART_STATE" task-checkoff "openspec/changes/<name>/tasks.md" "$OPENSPEC_TASK_TEXT"
```
...
- After final review passes, only the subagent dispatch loop is complete, not the Smart workflow. The coordinator must not load `finishing-a-development-branch` or pause to ask what comes next; it must return control to `smart-build` for exit checks, the phase guard, and phase handoff.
