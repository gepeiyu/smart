...
function buildSuperpowersInstallCommand(
  ...
  if (unknownIds.length > 0) throw new Error(`Unknown platform IDs: ${unknownIds.join(', ')}`);
  const agentNames = [...new Set(
    platformIds.map((id) => SKILLS_AGENT_MAP[id]).filter((name): name is string => Boolean(name)),
  )];
  if (agentNames.length === 0) throw new Error(`No skills CLI agent names resolved for platforms: ${platformIds.join(', ')}`);
  const args = ['skills', 'add', 'obra/superpowers', '-y'];
  if (scope === 'global') args.push('-g');
  for (const name of agentNames) args.push('--agent', name);
  return { command: getNpxExecutable(), args };
}

function buildLingmaSuperpowersStageCommand(): { command: string; args: string[] } {
  return { command: getNpxExecutable(), args: ['skills', 'add', 'obra/superpowers', '-y', '--agent', LINGMA_STAGE_AGENT] };
}
...
async function installSuperpowersForLingma(
  projectPath: string, scope: InstallScope,
  ...
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'smart-lingma-superpowers-'));
  try {
    const stageCommand = buildLingmaSuperpowersStageCommand();
    execFileSync(stageCommand.command, stageCommand.args, {
      cwd: tempDir, stdio: 'inherit', timeout: SUPERPOWERS_INSTALL_TIMEOUT_MS,
  ...
  if (skillsCliPlatformIds.length > 0) {
    ...
    try {
      execFileSync(command.command, command.args, {
        cwd: projectPath, stdio: 'inherit', timeout: SUPERPOWERS_INSTALL_TIMEOUT_MS,
        shell: process.platform === 'win32',
      });
    } catch (error) {
