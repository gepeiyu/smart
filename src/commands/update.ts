...
function getInstalledSmartSkillsDirs(
  baseDir: string,
  platform: Platform,
...
async function hasLocalSmartSkills(
  baseDir: string,
  platform: Platform,
  scope: InstallScope,
): Promise<boolean> {
...
async function detectInstalledSmartLanguage(
  baseDir: string,
  platform: Platform,
  scope: InstallScope = 'project',
...
async function detectInstalledSmartTargets(
  projectPath: string,
  options: DetectTargetsOptions = {},
): Promise<InstalledSmartTarget[]> {
  ...
  for (const scope of scopes) {
    ...
    for (const platform of PLATFORMS) {
      ...
      targets.push({
        scope,
        platform,
        language: await detectInstalledSmartLanguage(baseDir, platform, scope),
      });
    }
  }
  return targets;
...
async function detectSmartPackageScope(
  projectPath: string,
...
async function updateSmartNpmPackage(
  scope: InstallScope,
  projectPath: string,
...
export async function updateCommand(
  ...
  const workflow = await readProjectWorkflowSelection(projectPath);
  ...
  if (!options.skipNpm) {
    ...
    const npmUpdated = await updateSmartNpmPackage(
      packageScope,
      projectPath,
      log,
      options.json === true,
  ...
  for (const target of targets) {
    ...
    const languageSkillsDir = languageToSkillsDir(options.language, target.language);
    const { copied, skipped } = await copySmartSkillsForPlatform(
      baseDir,
      target.platform,
      true,
      languageSkillsDir,
    ...
    try {
      ...
      if (ruleCopied > 0)
        log(`  Smart rules -> ${target.platform.name}: ${ruleCopied} ${t(lang, 'rulesUpdated')}`);
    } catch (err) {
      log(
        `  Smart rules -> ${target.platform.name}: ${t(lang, 'rulesFailed')} (${(err as Error).message})`,
      );
    }
    if (target.platform.supportsHooks) {
      try {
        ...
        if (installed) {
          totalHooksInstalled++;
          log(`  Smart hooks -> ${target.platform.name}: ${t(lang, 'hooksUpdated')}`);
        } else if (reason)
          log(`  Smart hooks -> ${target.platform.name}: ${t(lang, 'hooksSkipped')} (${reason})`);
      } catch (err) {
        log(
          `  Smart hooks -> ${target.platform.name}: ${t(lang, 'hooksFailed')} (${(err as Error).message})`,
        );
      }
    }
  }
      ...
      JSON.stringify(
        {
          ...
          skills: { totalCopied, targets: targetResults },
  ...
  log(`    Workflow integrations: unchanged${workflow ? ` (${workflow.source})` : ''}`);
...
export {
  buildNpmUpdateArgs,
  detectSmartPackageScope,
  detectInstalledSmartLanguage,
  detectInstalledSmartTargets,
